...
Banco::Banco()
{
	PlaySound(_T("C:\\Users\\ADMIN\\Downloads\\nen (1).wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	diem = 0;
	level = 1;
	line = 0;
	tocdo = 1;
...
void Banco::capnhathinh(CClientDC* pdc)
{
	...
	if (thua == 1)
	{
		CString strthua;
		strthua.Format(_T("Game Over"));
		PlaySound(_T("C:\\Users\\ADMIN\\Downloads\\ketthuc (1).wav"), NULL, SND_FILENAME | SND_ASYNC);
		pdc->TextOutW(XDAU + 120, YDAU + CAO / 2 - 50, strthua);
	}
}
